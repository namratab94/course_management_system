	...
	if flask.session.get('logged_in'):
		if flask.request.method == 'POST':
			...
			try:
				...
				with db:
					cursor = db.cursor()
					sql = """Select User.ID as UserID, User.FName as FirstName, User.LName as LastName, Course.name as CourseName,\ 
...
Order by Material.ID """.format(int(flask.request.form['Input']), int(flask.request.form['Input']), int(flask.request.form['Input']))
